...
class Connect4(commands.Cog):
    ...
    async def play(self, interaction: discord.Interaction, opponent: discord.Member = None):
        if interaction.user in self.active_players:
            await interaction.response.send_message(f"{interaction.user.mention}, you are already in a game!", ephemeral=True)
            return

        if opponent in self.active_players:
            await interaction.response.send_message(f"{opponent.mention} is already in a game!", ephemeral=True)
            return
        elif opponent == None:
